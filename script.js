...
const api_key = `4d9bb11f933a493bb813a5c76ca07b06`

function test(the_src){
    VoiceRSS.speech({
        key:api_key,
        src:the_src,
        hl:'en-us',
...
const run = async ()=>{
    const response = await fetch(url).then(a=>{
        return a.json()
    })

    console.log(response.joke)
    document.getElementById('joke_text').innerHTML = response.joke
